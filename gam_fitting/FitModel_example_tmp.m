function [testFit,trainFit,param_mean] = FitModel_example_tmp(X,Xtype,Nprs,y,dt,h,nfolds,Lambda,linkfunc,invlinkfunc,Xname)
...
%     end
    ...
    fr_hat_test = invlinkfunc(test_X * param);
    nan_ok      = sum(test_X,2)==0;
    fr_hat_test(nan_ok) = nan;
    smooth_fr_hat_test = conv(fr_hat_test,h,'same')/dt; %returns vector same size as original
    
    subplot(nfolds,1,k)
    plot(smooth_fr_test)
    hold on
    plot(smooth_fr_hat_test)
    hold off
