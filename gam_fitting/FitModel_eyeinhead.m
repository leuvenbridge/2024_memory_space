function [testFit,trainFit,param_mean] = FitModel_eyeinhead(X,Xtype,Nprs,y,dt,h,nfolds,Lambda,linkfunc,invlinkfunc,Xname)
...
idx_ok  = find(sum(X,2)==1);
% divide the data up into nfolds*nchunks pieces
edges = round(linspace(1,numel(idx_ok)+1,nsections+1));
...
for k = 1:nfolds
    ...
    % get training data
    train_ind    = setdiff(idx_ok,test_ind);
