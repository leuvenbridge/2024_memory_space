function models = BuildGAM_Economic_eyeinhead(xt,yt,prs)
...
for i = 1:nvars
    ...
    if strcmp(xname{i},'SpatialHeading') || strcmp(xname{i},'SpatialView')...
            || strcmp(xname{i},'SpatialViewHD')
        [x{i}, xc{i}, nprs{i}]  = SpatialViewTo1hotConn_JL_DM(xt{i});
...
%     [models.testFit{n},models.trainFit{n},models.wts{n}] = FitModel_eyeinhead(X{n},Xtype{n},Nprs{n},yt,dt,h,nfolds,Lambda{n},linkfunc,invlinkfunc);
% end
...
for i = 1:nvars
    if i == 1  % select the best 1-variable model
        ...
        for n = modelinx
            ...
            [models.testFit{n},models.trainFit{n},models.wts{n}] = FitModel_eyeinhead(x_this,Xtype{n},Nprs{n},yt,dt,h,nfolds,Lambda{n},linkfunc,invlinkfunc,Xname{n}); % added Xname{n} 20200405
        end
        testFit = cell2mat(models.testFit);
        nrows = size(testFit,1);
...
%     fprintf('\t- Fitting model %d of %d (%s)\n', n, nModels,char('0'+ModelMatrix(:,n)));
%     x_this = cell2mat(x(Model{n}));
%     [models.testFit{n},models.trainFit{n},models.wts{n}] = FitModel_eyeinhead(x_this,Xtype{n},Nprs{n},yt,dt,h,nfolds,Lambda{n},linkfunc,invlinkfunc,Xname{n});
% end
...
for i=1:nModels
    for j=1:nvars
        if models.class{i}(j)
            if isempty(models.wts{i}(j~=1:nvars & models.class{i}))
                other_factors = 0;
            else
                other_factors = sum(cellfun(@(x,y) sum(x.*y), models.wts{i}(j~=1:nvars & models.class{i}), Px(j~=1:nvars & models.class{i})));
            end
            models.marginaltunings{i}{j} = invlinkfunc(models.wts{i}{j} + other_factors)/dt;
            if strcmp(xtype{j},'2D') && ~strcmp(xname{j},'SpatialHeading') && ~strcmp(xname{j},'SpatialView') && ~strcmp(xname{j},'SpatialViewHD')
